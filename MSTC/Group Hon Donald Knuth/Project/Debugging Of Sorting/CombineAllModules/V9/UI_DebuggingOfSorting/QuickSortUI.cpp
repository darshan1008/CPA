...
void AddDottedLineForLoop(HDC hdc,int x,int y,int CurrentLoop)
{
	...
	FormatedTextOut(hdc, x + 10, y + 2, (TCHAR*)("Loop Iteration : "));
	FormatedTextOut(hdc, x + 150 , y + 2, (TCHAR*)"%d", CurrentLoop);
...
int DrawQuickSort(int CurrentXPos, int CurrentYPos, HDC hdc, struct QuickSortData quicksortstruc)
{
	...
	if (quicksortstruc.isConditionDraw == FALSE && quicksortstruc.isIndexDraw == FALSE && quicksortstruc.isSwapElementDraw == FALSE)
	{
		...
		SetBoxesForArray(hdc, CurrentXPos , CurrentYPos + 40 ,N, -1, -1,quicksortstruc.arrayObj.pArr);
		return 70;
	}
