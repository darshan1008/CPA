...
void BubbleSortInMemory(int *pArr,int n)
{
	...
	for(arrObj.i=0; arrObj.i<n-1; arrObj.i++)
	{
		...
		for(arrObj.j=0; arrObj.j<n-1-arrObj.i; arrObj.j++)
		{
            memset(&obj,0,sizeof(obj));
            obj.passNumber = 0;
            obj.swapIndex1 = arrObj.j;
            obj.swapIndex2 = arrObj.j+1;

			if(arrObj.pArr[arrObj.j] > arrObj.pArr[arrObj.j+1])
			{
                obj.rangeObj.fromLine = 4; 
                obj.rangeObj.toLine = 4;
                obj.swapFlag = 1;
                copy_array(arrObj,&(obj.arrayObj),n);
                PushBackBubbleSort(g_bubbleVector,obj);
...
{
	BUBBLE_DATA* pTemp = NULL;
	pTemp = (BUBBLE_DATA*)realloc(pVector->pBubbleSort, (pVector->iVectorSize + 1) * sizeof(BUBBLE_DATA));
	if(NULL == pTemp)
    {
        puts("MEMORY ERROR");
        exit(-1);
    }
	pVector->pBubbleSort = pTemp;
    ...
    (pVector->pBubbleSort[pVector->iVectorSize]).swapIndex1 = Data.swapIndex1;
    (pVector->pBubbleSort[pVector->iVectorSize]).swapIndex2 = Data.swapIndex2;
